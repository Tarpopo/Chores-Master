...
public class ParticleManager : MySingleton<ParticleManager>
{
    ...
    public void PlayParticle(ParticleTypes particleType,Vector3 position,Vector3 rotation,Vector3 scale,Transform target,float delay=1f)
    {
        ...
        var particle = _managerPool.Spawn<Particle>(PoolType.Entities,particleSettings.Prefab,position,Quaternion.Euler(rotation));
        particle.PlayParticle(scale,target,delay,()=>_managerPool.Despawn(PoolType.Entities,particle.gameObject));
    }
}
[Serializable]
